...
class ActiveElectionsAdapter(private val activeElections: List<VotingModel>) :
    RecyclerView.Adapter<ActiveElectionsAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val viewButton: Button = itemView.findViewById(R.id.presidentialViewButton)
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val itemView = LayoutInflater.from(parent.context)
            .inflate(R.layout.election_list_voters, parent, false) // Replace "election_list_voters" with your actual layout file
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        isVoterRegistrationVerified(userId) { isVerified ->
            if (isVerified) {
                holder.viewButton.isEnabled = true
                holder.viewButton.setOnClickListener {
                    // Handle click event for the view button
                    val context = holder.itemView.context
                    val intent = Intent(context, VotingOptions::class.java)
                ...
                holder.viewButton.setText("Not Eligible")
            }
    ...
    private fun isVoterRegistrationVerified(userId: String, callback: (Boolean) -> Unit) {
        val database = FirebaseDatabase.getInstance()
        val reference = database.getReference("VoterRegistrations").child(userId)

        reference.addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                val registration = dataSnapshot.getValue(Registration::class.java)
                val isVerified = registration?.isVerified ?: false
                callback(isVerified)
            }
